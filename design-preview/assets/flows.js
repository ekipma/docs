...
window.createPastelFlows = function (app) {
  ...
  let ui = fresh();
  const labels = {login:'Welcome back',otp:'Verify your number',account:'A little about you',
  ...
  function page(name,id){
    if(['login','otp','account'].includes(name))return auth(name);
  ...
  function afterRender(name){
    ...
    const authPage=['login','otp','account'].includes(name);
    $('.phone').classList.toggle('flow-auth',authPage);
    if(authPage)$('#app-header').innerHTML=`<button class="header-back" data-go="${name==='login'?'welcome':'login'}">${icon('back')} Back</button><span class="brand">ekipma<span>•</span></span>`;
    if(name==='home'){
    ...
    if(name==='login'||name==='account')$('#page-description').textContent='Unified signup and login preview. Google and Apple simulate a successful provider response. Phone uses demo SMS code 123456. New users choose a display name; returning users continue home. No real authentication or SMS is performed.';
