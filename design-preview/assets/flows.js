...
window.createPastelFlows = function (app) {
  ...
  const fresh = () => ({hideZero:false, profile:{name:'Hayyaun',email:'hayyaun@example.com',photo:''},
    ...
    authPhone:'', signup:false, authError:'', otpError:'', sync:'online', clear:false, id:10, review:null});
  let ui = fresh();
  const labels = {login:'Welcome back',register:'Join Ekipma',otp:'Verify your number',account:'A little about you',
    ...
    stats:'Your patterns',notifications:'Notifications',invites:'Invited people',premium:'A little extra',shop:'Make it yours',tokens:'Add tokens',checkout:'Payment status',purchases:'Purchase history',policy:'Privacy & terms',states:'Everyday states',language:'Persian layout study'};
  ...
  function auth(page){
    if(page==='login')return `${title('YOUR PEOPLE ARE HERE','Welcome back.','A little less admin starts here.')}<div class="flow-auth-art" aria-hidden="true">✦</div><form id="login-form"><div class="flow-inline"><label class="form-label">Country<select class="field" name="country"><option>+98</option><option>+1</option><option>+44</option><option>+49</option></select></label>${field('Phone number','phone',ui.authPhone,'tel','required pattern="[0-9 +()-]{7,18}" autocomplete="tel-national" placeholder="912 345 6789"')}</div>${field('Password','password','','password','required minlength="6" autocomplete="current-password"')}${error(ui.authError)}${submit('Log in')}</form>${link('New here? Join Ekipma','register')}${link('Privacy & terms','policy')}`;
    if(page==='register')return `${title('ONE LITTLE STEP','Find your people.','Start with your phone number.')}<p class="flow-step">01 / 03 · Your number</p><form id="register-form"><div class="flow-inline"><label class="form-label">Country<select class="field" name="country"><option>+98</option><option>+1</option><option>+44</option></select></label>${field('Phone number','phone',ui.authPhone,'tel','required pattern="[0-9 +()-]{7,18}" autocomplete="tel-national" placeholder="912 345 6789"')}</div><label class="flow-check"><input type="checkbox" required> I agree to the privacy policy and terms.</label><button type="button" class="text-button" data-go="policy">Read privacy & terms</button>${submit('Send code')}</form>${link('Already have an account? Log in','login')}`;
    if(page==='otp')return `${title('CHECK YOUR MESSAGES','A tiny code.','Enter the six digits sent to '+(ui.authPhone||'your phone')+'.')}<p class="flow-step">02 / 03 · Verify</p><form id="otp-form">${field('Verification code','code','','text','required pattern="[0-9]{6}" maxlength="6" inputmode="numeric" autocomplete="one-time-code" placeholder="••••••"')}${error(ui.otpError)}${submit('Verify number')}</form>${button('Send a new code','resend','text-button')}${link('Change phone number','register')}`;
    return `${title('MAKE YOURSELF AT HOME','What should we call you?')}<p class="flow-step">03 / 03 · Your profile</p><form id="account-form">${field('Your name','name','','text','required maxlength="40" autocomplete="given-name"')}${field('Email','email','','email','required autocomplete="email"')}${field('Create password','password','','password','required minlength="8" autocomplete="new-password"')}<p class="form-note">At least 8 characters.</p>${submit('Let’s go')}</form>`;
  }
  ...
  function language(){return `<div lang="fa" dir="rtl" class="flow-persian">${title('کمی بیشتر شبیه تو','فضای تو.','پیش‌نمایش چیدمان فارسی')}<div class="flow-card"><h3>سلام، هیایون</h3><p class="subtext">حساب‌ها، نوبت‌ها و برنامه‌ها؛ کنار آدم‌های خودت.</p></div><div class="flow-stat-grid">${card('طلب داری','۲۱۴٬۰۰۰ تومان','mint')}${card('بدهکاری','۸۵٬۵۰۰ تومان','payment')}</div>${section('آدم‌های تو')}<div class="row"><span class="avatar">س</span><span class="row-copy"><strong>سارا</strong><small>دو حلقه مشترک</small></span><span class="row-end">۱۰۴٬۰۰۰ تومان</span></div>${section('تنظیمات')}<div class="setting-row"><span>زبان</span><strong>فارسی</strong></div><div class="setting-row"><span>فاصله فهرست</span><strong>باز و راحت</strong></div></div>${link('Back to English preferences','preferences')}`;}
  function page(name,id){
    ...
    if(name==='profile')return profile();if(name==='settings')return settings();if(name==='preferences')return preferences();if(name==='stats')return stats();if(name==='premium')return premium();if(name==='shop')return shop();if(name==='tokens')return tokens();if(name==='checkout')return checkout();if(name==='states')return statePage();if(name==='language')return language();
  ...
  function afterRender(name){
    ...
    if(name==='login'||name==='register')$('#page-description').textContent='Account access study. Enter sample details only; no credentials are transmitted or stored.';
    ...
    if(name==='language')$('#page-description').textContent='Persian typography, RTL alignment, and Toman formatting study. Full translation of every route remains a separate pass, as agreed.';
  ...
  document.addEventListener('submit', event=>{
    ...
    if(id==='register-form'){ui.authPhone=data.country+' '+data.phone;ui.signup=true;go('otp');}
    if(id==='otp-form'){if(data.code!=='123456'){ui.otpError='That code doesn’t match. Please try again.';render();return;}ui.otpError='';go('account');}
    if(id==='account-form'){if(!data.name.trim())return;ui.profile.name=data.name.trim();ui.profile.email=data.email;go('home');toast('Welcome to your Circles');}
  ...
  const menuGroups=[['Get started',[['login','Log in'],['register','Join Ekipma'],['otp','Verify number']]],['Your people',[['new-circle','Create Circle'],['join','Join Circle'],['add-friend','Add friend'],['contacts','Contacts'],['members/crew','Members'],['invite/crew','Invitation'],['circle-settings/crew','Circle settings']]],['Make it complete',[['record/coffee','A turn'],['repayment/sara','Confirm repayment'],['repayment/nima','Settle a balance'],['profile','Personal details'],['preferences','Preferences'],['stats','Your patterns'],['notifications','Notifications'],['invites','Invited people'],['premium','Ekipma Plus'],['shop','The little shop'],['tokens','Add tokens'],['checkout','Payment status'],['purchases','Purchase history'],['policy','Privacy & terms'],['language','Persian study'],['states','Loading, offline & empty']]]];
  ...
  return {page,append,afterRender,click,label:name=>labels[name]||name,hideZero:()=>ui.hideZero,
    ...
    parent:(p,id)=>['members','invite','circle-settings'].includes(p)?'group/'+id:p==='repayment'?'friend/'+id:['profile','preferences','stats','premium','shop','notifications','invites','language'].includes(p)?'settings':['tokens','purchases'].includes(p)?'shop':p==='checkout'?'tokens':p==='review'?'add':['new-circle','join','add-friend','contacts'].includes(p)?'circles':null,
